...
exports.registerEmployee = async (req, res) => {
    ...
    // console.log("Received file:", req.file); // Aktifkan untuk debugging
    ...
    try {
        ...
        // Hapus file jika ada error parsing
        if (req.file) {
            fs.unlinkSync(req.file.path);
        }
        return res.status(400).json({ message: 'educationHistory/trainingHistory must be in valid JSON format' });
    }
    ...
    try {
        ...
        const newEmployee = new Employee({
            ...
            phone_nmb,
            email,
            photo: req.file ? `/uploads/profile-photos/${req.file.filename}` : undefined, // Ubah path disini
            ktp_address,
            educationHistory,
            trainingHistory,
            salarySlip,
        ...
        console.log('Employee registered successfully with photo:', newEmployee.photo);
        
        res.json({ 
            message: 'Employee registered successfully',
            photoPath: newEmployee.photo // Tambahkan info photo di response
        });
    } catch (err) {
...
// Get Employee Personal Data Method
...
exports.checkIn = async (req, res) => {
    try {
      ...
      // Checking if employee already check-in but haven't check-out
      ...
      // Checking for attendace record in a day
...
// Get Employee's Attendance Information Method
...
// Create Employee Leave Request Method
exports.leaveRequest =  async (req, res) => {
    try {
      ...
      // Validate if employee remaining leaves enough
...
// Get Employee's Leave Requests Method
...
// Delete Employee's Leave Request Method
...
// Get Employee's Salary Slip Method
...
// Download Employee's Salary Slip Method
exports.downloadSalarySlipToClient = async (req, res) => {
    try {
        ...
        const slipPath = path.join(__dirname, '../utils', employee.salarySlip); 
        const fileName = path.basename(employee.salarySlip);

        // Checking if the file available
...
// Change Employee's Password Method
...
// Get Leave Information of Logged-In Employee
...
// Add Employee's Education Method 
...
// Delete Specific Education Method
...
// Add Employee's Training Method
...
// Delete Specific Training Method
